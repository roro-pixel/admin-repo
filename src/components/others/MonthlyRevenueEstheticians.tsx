...
interface MonthlyRevenueEstheticiansProps {
  onBack: () => void;
}

const MonthlyRevenueEstheticians: React.FC<MonthlyRevenueEstheticiansProps> = ({ onBack }) => {
  const { estheticianAppointments } = useAppointmentEstheticians();

  const revenueData = useMemo(() => {
    if (!estheticianAppointments.data) return [];

    const monthlyData = estheticianAppointments.data.reduce((acc, appointment) => {
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <h2 className="text-2xl font-bold">Revenu mensuel (Esthétique)</h2>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm">
        <h3 className="text-lg font-semibold mb-4">Évolution des revenus (Esthétique)</h3>
        <div className="h-96">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={revenueData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              />
              <Bar dataKey="revenue" fill="#DD6B9D" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm overflow-x-auto">
        <h3 className="text-lg font-semibold mb-4">Détails mensuels (Esthétique)</h3>
...
export default MonthlyRevenueEstheticians;